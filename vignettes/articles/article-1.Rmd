...
ccc_graph_list <-
  compute_spatial_ccc_graph_list(spe = spe_brain,
                                 ...
                                 # below should be commented before submitting it to GitHub; 
                                 # otherwise, GitHub will fail.
                                 workers = 8,
                                 LRdb = LRdb_m)
...
ccc_tbl_between_clusters %>%
  dplyr::arrange(LR, desc(LRscore.median))
```

```{r}
ccc_tbl_between_clusters %>%
  dplyr::arrange(mclust_8_src, mclust_8_dst, desc(n))
```
