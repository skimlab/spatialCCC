...
x <-
  ccc_tbl %>%
  left_join(cell_annot_df %>%
              select(cell_id, starts_with("mclust_")),
            by = c("src" = "cell_id")) %>%
  dplyr::rename_with(~paste0(.x, "_src"),
                     starts_with("mclust_")) %>%
  left_join(cell_annot_df %>%
              select(cell_id, starts_with("mclust_")),
            by = c("dst" = "cell_id")) %>%
  dplyr::rename_with(~paste0(.x, "_dst"),
                     starts_with("mclust_") & !ends_with("_src"))
```
